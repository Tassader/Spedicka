...
SELECT     p.ID, 'Praha' AS Stredisko, p.Cislo AS Pøeprava, p.Smer, NakladySumy.Naklad, VynosySumy.Vynos, 
                      VynosySumy.Vynos - NakladySumy.Naklad AS Zisk, p.Rok, p.Mesic AS Mìsíc, dbo.Firma.Firma AS Zákazník, dbo.Obchodnik.Jmeno AS Obchodník, 
...
AS
SELECT     p.ID, 'AM Trucking' AS Stredisko, p.Cislo AS Pøeprava, p.Smer, NakladySumy.Naklad, VynosySumy.Vynos, 
...
SELECT p.*, an.CastkaKC AS NakladyKC, av.CastkaKC AS VynosyKC, av.CastkaKC-ISNULL(an.CastkaKC,0) AS ZiskKC, DuplicitniZakRef, DatumCasVykladky,
    Stav2=CASE 
        WHEN CURRENT_TIMESTAMP < ETS THEN 'PÅ™ed odeslÃ¡nÃ­m'
        WHEN CURRENT_TIMESTAMP < ETA THEN 'Na cestÄ›'
        WHEN CURRENT_TIMESTAMP < DatumCasVykladky THEN 'Na doruÄenÃ­'
        WHEN CURRENT_TIMESTAMP >= DatumCasVykladky THEN 'DoruÄena'
    END
